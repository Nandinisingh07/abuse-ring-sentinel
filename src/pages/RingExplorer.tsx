...
import { Network, Search, ShieldAlert, Cpu, Info } from 'lucide-react';
...
export default function RingExplorer() {
  ...
  // so that whenever two accounts genuinely share a cluster in our data,
  // both are guaranteed to be present and a connection can be drawn,
  // regardless of which tier/search/size filter is active.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (expandedClusterId !== null) {
      ...
      // Filter clusters list to keep only those present in activeClusterIds
      const activeClusters = clusters.filter(c => activeClusterIds.has(c.cluster_id));
      
      nodes = activeClusters.map(c => {
        // Resolve cluster group risk tier based on its active members
    ...
    if (expandedClusterId !== null) {
      ...
      simulation = d3Force.forceSimulation<GraphNode>(nodes)
        ...
        .force("collision", d3Force.forceCollide().radius((d: any) => {
          ...
          return 12 + Math.sqrt(size) * 8 + 4; // size-dependent collision radius
        }));
    }
  ...
  }, [loading, accounts, clusters, minClusterSize, selectedTier, searchQuery, expandedClusterId]);
  ...
  const drawGraph = (canvas: HTMLCanvasElement, nodes: GraphNode[]) => {
    ...
    // Hash function to get a stable, distinct HSLA color for each cluster
    const getClusterColor = (clusterId: number) => {
      let hash = 0;
      const idStr = clusterId.toString();
      for (let i = 0; i < idStr.length; i++) {
        hash = idStr.charCodeAt(i) + ((hash << 5) - hash);
      }
      const hue = Math.abs(hash) % 360;
      const saturation = isLight ? "85%" : "75%";
      const lightness = isLight ? "42%" : "60%";
      const opacity = isLight ? "0.7" : "0.55";
    ...
    // 2. Draw Nodes (overview cluster bubbles or detail user nodes)
    nodes.forEach(node => {
  ...
  const handleSelectCluster = async (clusterId: number) => {
    try {
      ...
      setExpandedClusterId(clusterId); // Expand on graph
      setSelectedNode(null);
      setNodeExplanation(null);
    } catch (err) {
  ...
  return (
    <div className="page-container py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Canvas Area */}
        <div className="lg:col-span-2 bg-brand-panel border border-brand-border rounded-lg relative overflow-hidden h-[450px] flex items-center justify-center">
          {expandedClusterId !== null && (
            <button
              ...
              className="absolute top-4 left-4 px-3 py-1.5 bg-brand-bg hover:bg-brand-panel-light border border-brand-border text-slate-300 hover:text-slate-100 rounded text-xs font-mono font-semibold transition-colors cursor-pointer flex items-center gap-1.5 z-10"
            >
              <span>← Collapse Ring View</span>
            </button>
          )}
          ...
          {selectedNode ? (
            <div className="space-y-4">
              <div className="border-b border-brand-border pb-3">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className={`px-1.5 py-0.5 rounded text-[8px] font-mono font-bold ${
                    selectedNode.risk_tier === "LOW" ? "bg-green-500/10 text-green-600 dark:text-green-400" :
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-1">
                  <span className="text-[10px] font-bold font-mono text-slate-500 uppercase">SHAP Risk Breakdown</span>
                ...
                {nodeExplanation ? (
                  ...
                  <div className="text-xs text-slate-500 italic font-sans py-2">
                    No SHAP explanation available for this account.
                  </div>
                )}
              </div>
                  ...
                  {selectedCluster.members.map((member) => (
                    ...
                    >
                      ...
                      <span className={`font-semibold ${
                        member.risk_tier === "LOW" ? "text-green-600 dark:text-green-400" :
            ...
            <div className="flex flex-col items-center justify-center flex-grow text-center text-slate-500 p-4">
              ...
              <p className="text-xs font-sans">
                Click on any node in the graph to display its features and SHAP explanation, or click below to browse clusters.
              </p>
